...
%GUITEMPLATE(...) template for GUIs. Click on "View code for
%mygui.GUItemplate" and save with a new name.
%   arg: explanation
...
%         S.(varname) = floor(str2double(source.String));
    ...
    %LN_CALLBACK(source,~,which,varname) callback for lines on graphs.
    %   source: handle of object that called back
    %   which: index for handle arrays
    %   varname: name of field of struct S to update.
        [x, ~] = ginput(1);
...
%  Utility functions
    ...
    %   which = [row, column] numbers
    %   maxwhich = number of [rows columns]
